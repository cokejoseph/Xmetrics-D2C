...
type TabType = 'all' | 'review'

export default function OrderList() {
  const { orders, approveOrder, holdOrder } = useAppStore()
  ...
  const filtered = useMemo(() => {
    ...
    if (tab === 'review') {
      list = list.filter(o => o.rto_review_status === 'PENDING' || o.rto_review_status === 'HELD')
    }
  ...
  }, [orders, tab, search, filterChannel, filterPayment, filterFulfillment, filterRTO])
    ...
    <div className="space-y-4">
      ...
      {highRiskPending > 0 && (
        ...
        >
          ...
          <span>{highRiskPending} high-RTO order{highRiskPending > 1 ? 's' : ''} pending review — check the Review Queue</span>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        <TabBtn active={tab === 'all'} onClick={() => setTab('all')}>All Orders</TabBtn>
        <TabBtn active={tab === 'review'} onClick={() => setTab('review')} badge={pendingReviewCount}>
          Review Queue
        </TabBtn>
      </div>
        ...
        <td className="px-4 py-3">
          {order.rto_review_status === 'PENDING' || order.rto_review_status === 'HELD' ? (
            <div className="flex gap-1">
              ...
              <button
                onClick={onHold}
                className="px-2 py-1 text-xs font-medium text-amber-700 bg-amber-50 rounded-lg hover:bg-amber-100 transition-colors"
              >
                Hold
              </button>
            </div>
